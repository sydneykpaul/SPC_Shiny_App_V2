...
## Group raw data
groupData <- function(df, rare_event) {
  ...
  if(rare_event) {
    ...
    aggregate(
      ...
      by = list(df$subgroup_row),
      FUN = function(x) {max(as.character(x), na.rm = T)}
    ))
    suppressWarnings(
    dfout <-
      data.frame(
...
## Calculate data group median
gmedian <- function(df, rateMultiplier) {
  stats::median((df$num_count / df$denom_count) * rateMultiplier)
}
...
calculateSbar <- function(chart_type, df, grp, gsbar, gcount, mean) {
  if (chart_type == 'p-chart' | chart_type == 'p\'-chart') {
    sbar <- sqrt(mean*(1-mean)) # binomial
  } else if (chart_type == 'u-chart' | chart_type == 'u\'-chart') {
    sbar <- sqrt(mean) # poisson
  } else {
    sbar <- rep(sapply(split(df, grp), gsbar), gcount)  # normal
  }
  return(sbar)
}
